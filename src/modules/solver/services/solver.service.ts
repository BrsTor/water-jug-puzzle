...
export class SolverService {
    ...
    async findSolution(puzzle: PuzzleDto) {

        const cacheKey = `${puzzle.x_capacity}-${puzzle.y_capacity}-${puzzle.z_amount_wanted}`;
        const cachedSolution = await this.cacheManager.get(cacheKey);

        if (cachedSolution) {
            return { solution: cachedSolution };
        }
        ...
        // Cache the solution
        await this.cacheManager.set(cacheKey, solution); // Cache for 5 minutes
